...
sheet = wb["Sheet"]
print(sheet.max_row)
first_column=sheet.cell(row=i,column=1).value
for i in range(1,23758):
    if first_column=='Garlic':
        sheet.cell(row=i,column=2)=3.07
    elif sheet.cell(row=i,column=1).value=='Celery':
        sheet.cell(row=i,column=2)=1.19
    elif sheet.cell(row=i,column=1).value=='Lemon':
        sheet.cell(row=i,column=2)=1.27
print('Done.')

...
# Access by row & column numbers (1-based index)
c = sheet.cell(row=1, column=2)   # B1
print(f"Row {c.row}, Column {c.column} is {c.value}")

print(f"Cell {c.coordinate} is {c.value}")

# Direct access another cell
print("Cell A1:", sheet.cell(row=1, column=1).value)
print("Cell C1:", sheet.cell(row=1, column=3).value)

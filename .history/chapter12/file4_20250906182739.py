...
import openpyxl
from openpyxl.chart import Reference,Ser,BarChart,LineChart,ScatterChart
wb = openpyxl.Workbook()
sheet = wb.active
for i in range(1, 9):
    sheet["A" + str(i)] = i

refObj=op(sheet,min_row=1,min_col=1,max_col=1)
srObj=series(refObj,title='First Series')
chrObj=BarChart()
chrObj.append(srObj)

chrObj.height=20   # this means the height of chart is 20 rows
chrObj.width=10   # this means the width of chart is 10 rows
